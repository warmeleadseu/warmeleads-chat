-- ============================================================================
-- 129 — Airco Partners: bestaande leads → prospects (zelfde patroon als 096)
-- ============================================================================

WITH moved AS (
  DELETE FROM leads
  WHERE branch = 'airco_partners'
  RETURNING *
),
ins AS (
  ...
  SELECT
    ...
    ARRAY['airco_partners'::text],
    'nieuw',
    'meta_partner',
    jsonb_strip_nulls(
      jsonb_build_object(
        'partner_branch', 'airco_partners',
    ...
    CASE
      WHEN NULLIF(TRIM(m.notities), '') IS NOT NULL THEN
        TRIM(m.notities) || E'\n\n(Gemigreerd van leads / Airco Partners.)'
      ELSE 'Gemigreerd van leads (Airco Partners).'
    END,
    m.created_at,
    now()
  FROM moved m
...
SELECT
  ...
  NULL,
  'import',
  'Airco Partners (gemigreerd)',
  'Deze prospect kwam binnen als lead en is automatisch naar de prospect-pijplijn verplaatst.',
  jsonb_build_object('migration', '129_airco_partners_leads_to_prospects')
FROM ins i;
