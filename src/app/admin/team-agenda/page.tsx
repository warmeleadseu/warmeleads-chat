    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="ml-2 min-w-0 max-w-[180px] truncate text-sm font-bold text-slate-800 sm:max-w-none">{viewLabel(view, anchor)}</span>
          {loading && (
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex w-full overflow-hidden rounded-md ring-1 ring-slate-200 sm:w-auto">
            <button
              onClick={() => setView('month')}
              className={`inline-flex flex-1 items-center justify-center gap-1 px-2.5 py-1.5 text-xs font-semibold sm:flex-none ${
      ...
      {confirmToast && (
        <div className="fixed bottom-5 left-4 right-4 z-[60] flex items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-800 shadow-lg sm:left-auto sm:right-5 sm:max-w-sm" style={{ bottom: 'max(1.25rem, env(safe-area-inset-bottom))' }}>
