...
// Headers die we automatisch overslaan: meta-kolommen die niets met de
// lead zelf te maken hebben (CRM-eigenaar, technische audit-kolommen, etc.).
const SKIP_PATTERNS = [
  /afstand/i, /binnen\s*gebied/i, /verwerkt/i, /geexporteerd/i, /ge[eë]xporteerd/i,
  /nieuwsbrief/i, /newsletter/i,
  /^eigenaar$/i, /^owner$/i, /^assigned\s*to$/i, /^toegewezen\s*aan$/i,
  /^id$/i, /^uuid$/i, /^row[\s_-]?id$/i,
];

const ALIASES: Record<string, string[]> = {
  naam_klant: [
    'naamklant', 'klantnaam', 'volledigenaam', 'naambewoner', 'voornaam', 'achternaam',
    'naam', 'name', 'fullname', 'contactnaam', 'contact', 'contactpersoon', 'klant',
  ],
  email: [
    'emailadres', 'emailaddress', 'mailadres', 'mailadress', 'email', 'emal', 'mail',
    'emailadress', 'emailadressen',
  ],
  telefoonnummer: [
    'telefoonnummer', 'telefoon', 'phonenumber', 'phone', 'mobiel', 'mobielnummer',
    'mobile', 'mobilenumber', 'mobilephone', 'gsmnummer', 'gsm', 'telnr', 'tel',
    'primairtelefoonnummer',
  ],
  postcode: ['postcode', 'zipcode', 'postalcode', 'postal', 'zip', 'pc'],
  huisnummer: [
    'huisnummer', 'huisnr', 'housenumber', 'huisnummertoevoeging', 'streetnumber',
    'huisnummermettoevoeging',
  ],
  plaatsnaam: ['plaatsnaam', 'woonplaats', 'plaats', 'stad', 'city', 'gemeente', 'town'],
  provincie: ['provincie', 'province', 'regio', 'region'],
  wervingsdatum: [
    // Algemeen / NL
    'wervingsdatum', 'aanvraagdatum', 'invuldatum', 'datuminteresseklant', 'datum',
    'aanmeld', 'aanmeldatum', 'aanmelddatum', 'aanmeldingsdatum', 'inschrijfdatum',
    'aangemaakt', 'aangemaaktop', 'gemaakt', 'gemaaktop', 'gemaaktopdatum',
    // Engels
    'date', 'created', 'createdat', 'createdate', 'createdon', 'datecreated',
    'submitted', 'submittedat', 'submission', 'submissiondate', 'datesubmitted',
    'entrydate', 'entry', 'recordedat', 'leaddate',
  ],
  notities: [
    'notities', 'opmerkingen', 'opmerking', 'notes', 'note', 'toelichting', 'extra',
    'comment', 'comments', 'description', 'omschrijving', 'remarks',
    // Vaak geleverd door CRM-exports — we behouden ze als notitie zodat ze
    // niet stilletjes verdwijnen bij het importeren.
    'labels', 'label', 'tags', 'tag', 'formulier', 'form', 'formname',
    'kanaal', 'channel',
  ],
  bron: [
    'bron', 'source', 'leadbron', 'leadsource', 'campagne', 'campaign',
    'utmsource', 'medium', 'utmmedium',
  ],
  status: [
    'status', 'fase', 'phase', 'stage', 'state', 'leadstatus',
  ],
...
export default function ImportPage() {
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    // `cellDates: true` zorgt dat Excel-datumcellen als `Date`-objecten
    // binnenkomen i.p.v. als ondoorgrondelijke serial getallen — dat
    // `parseImportDate` voorheen vaak verkeerd interpreteerde. `raw: false`
    // formatteert overige cellen netjes (geen drijvende-komma-spookjes).
          ...
          <div className="p-6">
            <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {(() => {
                  const skipped = currentMappingSheet.headers.filter(
                    h => !currentMappingSheet.mapping[h] && !SKIP_PATTERNS.some(p => p.test(h))
                  );
            ...
            {(() => {
              const sheetIdx = sheets.indexOf(currentMappingSheet);
              const skippedWithData = currentMappingSheet.headers.filter(h => {
                if (currentMappingSheet.mapping[h]) return false;
                if (SKIP_PATTERNS.some(p => p.test(h))) return false;
                return currentMappingSheet.rows.some(r => String(r[h] || '').trim() !== '');
              });
              if (skippedWithData.length === 0) return null;

              const notitiesHeader = Object.entries(currentMappingSheet.mapping).find(([, f]) => f === 'notities')?.[0];

              const archiveAsNotities = () => {
                if (skippedWithData.length === 0) return;
                setSheets(prev => {
                  const next = [...prev];
                  const s = { ...next[sheetIdx], mapping: { ...next[sheetIdx].mapping }, rows: [...next[sheetIdx].rows] };

                  // We voegen de waardes van alle ongekoppelde kolommen samen
                  // in een synthetische `__samenvoeging__notities__` kolom en
                  // koppelen die aan het notities-veld. Bestaande notities-
                  // inhoud blijft behouden en wordt vooraan gehangen.
                  const SYNTH_KEY = '__warmeleads_archive_notities__';

                  s.rows = s.rows.map(row => {
                    const parts: string[] = [];
                    if (notitiesHeader) {
                      const existing = String(row[notitiesHeader] || '').trim();
                      if (existing) parts.push(existing);
                    }
                    for (const h of skippedWithData) {
                      const v = String(row[h] || '').trim();
                      if (v) parts.push(`${h}: ${v}`);
                    }
                    const combined = parts.join(' · ');
                    return { ...row, [SYNTH_KEY]: combined };
                  });

                  if (!s.headers.includes(SYNTH_KEY)) {
                    s.headers = [...s.headers, SYNTH_KEY];
                  }

                  // Verwijder oude notities-mapping en wijs onze samengevoegde
                  // kolom toe aan notities.
                  for (const k of Object.keys(s.mapping)) {
                    if (s.mapping[k] === 'notities') delete s.mapping[k];
                  }
                  s.mapping[SYNTH_KEY] = 'notities';
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div className="text-xs text-amber-900">
                        <p className="font-semibold">
                          {skippedWithData.length} kolom{skippedWithData.length === 1 ? '' : 'men'} met data wordt nu niet meegeïmporteerd
                        </p>
                        <p className="mt-0.5 text-amber-800">
                          {skippedWithData.slice(0, 6).map(h => <code key={h} className="mr-1 rounded bg-white/70 px-1.5 py-0.5 text-[10px] text-amber-900">{h}</code>)}
                          {skippedWithData.length > 6 && <span className="text-amber-700/80">+ {skippedWithData.length - 6} meer</span>}
                        </p>
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={archiveAsNotities}
            ...
            <div className="space-y-2">
              {currentMappingSheet.headers.map(header => {
                ...
                const autoSkipped = !crmField && SKIP_PATTERNS.some(p => p.test(header));
                const isSynthetic = header.startsWith('__warmeleads_');
                const displayHeader = isSynthetic ? 'Samengevoegde data (overgeslagen kolommen)' : header;
                return (
                  <div key={header} className={`flex flex-col gap-2 rounded-lg border px-4 py-3 sm:flex-row sm:items-center sm:gap-3 sm:py-2.5 ${crmField ? 'border-slate-100 bg-slate-50/50' : autoSkipped ? 'border-slate-100 bg-slate-50/30 opacity-60' : 'border-amber-200 bg-amber-50/40'}`}>
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-medium text-slate-700">{displayHeader}</p>
                    ...
                    <select
                      ...
                      className={`w-full rounded-lg border px-3 py-2.5 text-sm sm:w-48 sm:shrink-0 sm:py-1.5 ${crmField ? 'border-emerald-300 bg-emerald-50 text-emerald-700' : autoSkipped ? 'border-slate-200 bg-white text-slate-400' : 'border-amber-300 bg-white text-amber-700'}`}
                    >
                      <option value="">- Overslaan -</option>
                      {fields.map(f => (
                        <option
            ...
            <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex gap-3">
                {mappingActiveSheets.length > 1 && activeTab < mappingActiveSheets.length - 1 ? (
                  <button onClick={() => setActiveTab(activeTab + 1)} className="w-full rounded-lg bg-button-gradient px-5 py-2.5 text-sm font-bold text-white shadow-sm sm:w-auto">
                  ...
                  <button
                    ...
                    className="w-full rounded-lg bg-button-gradient px-5 py-2.5 text-sm font-bold text-white shadow-sm disabled:opacity-50 sm:w-auto"
                  >
          ...
          {results.map((r, i) => (
            <div key={i} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-semibold text-slate-800">{r.sheetName}</p>
                  <p className="truncate text-xs text-slate-500">{r.branch}</p>
                </div>
