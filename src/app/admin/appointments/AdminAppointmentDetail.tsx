...
import {
  XMarkIcon, PhoneIcon, EnvelopeIcon, MapPinIcon, CalendarDaysIcon, ClockIcon,
  UserIcon, CheckCircleIcon, ExclamationCircleIcon, PencilSquareIcon, TrashIcon,
  ChatBubbleBottomCenterTextIcon,
} from '@heroicons/react/24/outline';
import type { AdminAppointment } from './page';

interface Customer { id: string; name: string }
interface BranchOption { slug: string; name: string }

const STATUS_DOT: Record<string, string> = {
  scheduled: 'bg-indigo-500', completed: 'bg-emerald-500', no_show: 'bg-rose-500',
...
export default function AdminAppointmentDetail({
  appointment, customers, branches, onClose, onUpdated,
}: {
  ...
  const customerName = appointment.customers?.name || customers.find(c => c.id === appointment.customer_id)?.name;
  const branchName = branches.find(b => b.slug === appointment.branch)?.name || appointment.branch;

  const save = async () => {
    setSaving(true); setErr(null);
    try {
      const res = await adminFetch(`/api/admin/appointments/${appointment.id}`, {
        method: 'PATCH',
        body: JSON.stringify({
  ...
  const doDelete = async () => {
    setSaving(true);
    try {
      const res = await adminFetch(`/api/admin/appointments/${appointment.id}`, { method: 'DELETE' });
      if (res.ok) onUpdated();
    } finally { setSaving(false); }
  };
      ...
      >
        <header className="flex items-start justify-between border-b border-slate-200 px-5 py-4 sm:rounded-t-2xl" style={{ paddingTop: 'max(env(safe-area-inset-top), 1rem)' }}>
          <div className="min-w-0 flex-1">
            <div className="mb-1 flex items-center gap-2">
              <span className={`h-2 w-2 rounded-full ${STATUS_DOT[appointment.status]}`} />
            ...
            <p className="truncate text-xs text-slate-500">{customerName} · {branchName}</p>
          </div>
          ...
          <section className="mb-4 rounded-xl border border-slate-200 bg-gradient-to-br from-brand-purple/5 to-brand-pink/5 p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-bold text-slate-900">{s.toLocaleDateString('nl-NL', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</p>
              ...
              <section className="space-y-2">
                ...
                {appointment.contact_phone && (
                  <a href={`tel:${appointment.contact_phone}`} className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-700 hover:border-slate-300">
                    <PhoneIcon className="h-4 w-4 text-slate-400" /><span className="flex-1">{appointment.contact_phone}</span>
                  </a>
                )}
              ...
              {appointment.portal_users?.name && (
                <section className="mt-4">
                  ...
                  <div className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-700">
                    ...
                    <span>{appointment.portal_users.name}</span>
                  </div>
                </section>
              )}
              ...
              {appointment.status === 'scheduled' && (
                <section className="mt-5">
                  ...
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                    <button onClick={() => changeStatus('completed')} disabled={saving} className="flex items-center justify-center gap-1.5 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2.5 text-sm font-semibold text-emerald-700 hover:bg-emerald-100">
        ...
        <footer className="flex items-center gap-2 border-t border-slate-200 bg-white px-5 py-3" style={{ paddingBottom: 'max(env(safe-area-inset-bottom), 0.75rem)' }}>
          {!editing ? (
            ...
            <>
              <button onClick={() => setEditing(false)} className="h-11 flex-1 rounded-xl border border-slate-200 bg-white text-sm font-semibold text-slate-700 hover:bg-slate-50">Annuleren</button>
